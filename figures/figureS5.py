...
Creates figure S5: Spectral densities for weak coupling perturbation theory
"""
...
for i in range(len(paths)):
    ...
    freq = npz['freqs_weak_1']
    Kw = npz["Kw_ave_weak1"]
    freq_1.append(freq)
    Kw_weak1.append(Kw)
    
    freq = npz['freqs_weak_2']
    Kw = npz["Kw_ave_weak2"]
    freq_2.append(freq)
    Kw_weak2.append(Kw)
...
axs[0].set_title(r"""$\omega_{as} = 100$ cm$^{-1}$
                     $\tilde{\omega}_{s} = 105$ cm$^{-1}$""",fontsize=26,y=1.05)
axs[1].set_title(r"""$\omega_{as} = 150$ cm$^{-1}$
                     $\tilde{\omega}_{s} = 112$ cm$^{-1}$""",fontsize=26,y=1.05)
axs[2].set_title(r"""$\omega_{as} = 200$ cm$^{-1}$
                     $\tilde{\omega}_{s} = 122$ cm$^{-1}$""",fontsize=26,y=1.05)
axs[2].legend(frameon=False,fontsize=18)

fig.tight_layout()
plt.savefig("./figureS5/figureS5.pdf",bbox_inches="tight")
plt.show()
